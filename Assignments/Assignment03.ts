...
const discount: string = priceOfProduct > 100 ? `The discount for this product is 10%` : `The discount for this product is 5%`
...
// another way:
const isLeapYear: boolean = (year % 4 && year % 100 !== 0) || (year % 400 === 0);
console.log(`Is ${year} a leap year? : `+isLeapYear);


//Question-07:
// - Develop a program that determines the day of the week. Ask the user for a number (1-7) and use nested if statements to print the corresponding day's name.
const userInputNumber: number = 3;
...
let attendance: number = 90; //out of 100
let practicalMarks: number = 50; //out of 60
let practicalTotal: number = 60;
let theoryMarks: number = 40; //out of 50
let theoryTotal: number = 50;
let attendancePercentage: number = Math.floor((attendance / 100) * 100);
let practicalPercentage: number = Math.floor((practicalMarks / practicalTotal) * 100)
let theoryPercentage: number = Math.floor((theoryMarks / theoryTotal) * 100);
...
let score: number = 35;
let totalMarks: number = 60;
let percentageScore: number = Math.floor((score / totalMarks) * 100);
