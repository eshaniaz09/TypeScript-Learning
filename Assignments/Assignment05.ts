...
var ATM = (pinNumber:number , accountNumb:number , withdrawAmount_:number , totalBalance_:number) : void=>
{
   if (pinNumber === 2244) {
...
let timeCurrent: number = new Date().getHours();
var greeting = (a:number): void => {
   if (a >= 4 && a <= 11) {
...
// var classN: number = 2;
var studyClass = (classN:number): void => {
   switch (classN) {
      case 1:
...
var maximumMinimum = (a:number , b:number, c:number , d:number, e:number):number => {
   let maxNumb: number = a;
   if (b > maxNumb) {
      maxNumb = b;
      }
   if (c > maxNumb) {
...
//way 1:
var percentCalculator = (totaL: number, obtained: number): number => {
   var per: number = (obtained / totaL) * 100;
   return per;
} 
// way 2:   
...
var discountOfProduct = (actualPrice: number): number => {
   var discountPrice1: number = actualPrice - ((actualPrice / 100) * 10);
   var discountPrice2: number = actualPrice - ((actualPrice / 100) * 5);
   if (actualPrice > 100) {
...
var userProvidedAge = (agE: number): string => {
...
var temperatureType = (temperature:number): string => {
   var clothSuggestion: string;
   if (temperature < 10) {
...
var numberCheck = (num_: number): boolean => (num_ % 3 === 0);
console.log(numberCheck(18));
// to check divisible of 5:
var numberCheck = (num_: number): boolean => (num_ % 5 === 0);
console.log(numberCheck(18));
//for both check
var numberCheck = (num_: number): boolean => (num_ % 3 === 0 && num_ % 5 === 0);
console.log(numberCheck(18));
...
var checkLeapYear = (yr:number): boolean=> ((yr % 4 === 0 && yr % 100 != 0) || (yr % 400 === 0)); 
...
var daysOfWeek = (dayNum:number): string => {
   var dayName: string;
   switch (dayNum) {
      case 1:
         dayName = 'Monday';
...
var priceIncludingTax = (pricePerUnit: number, unitSConsumed: number) => {
...
var studentPassOrFail = (attendance_Percentage:number, theory_Percentage:number, practical_Percentage:number ): string=>{
...
var gradesOfStudents = (percentAge:number): string => {
   if (percentAge >= 80) {
      return 'A';
